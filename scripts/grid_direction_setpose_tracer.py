...
INTERSECTION_COOLDOWN_TICKS = 5
INTERSECTION_LOST_REQUIRED_TICKS = 20
...
FORWARD_SPEED_MAX = 0.28
FORWARD_SPEED_MIN = 0.08
...
class GridDirectionSetPoseTracer(Node):
    ...
    def do_takeoff(self):
        ...
        self.get_logger().info("이륙 완료 → 앞으로 3m 이동 시작")
    ...
    def do_move_forward_3m(self):
        ...
        if self.forward_moved < MOVE_FORWARD_DISTANCE:
            vx = FORWARD_UNIT_X * MOVE_FORWARD_SPEED
            vy = FORWARD_UNIT_Y * MOVE_FORWARD_SPEED

            self.apply_velocity(vx, vy, 0.0)
            self.forward_moved += MOVE_FORWARD_SPEED * self.dt
        ...
        self.state = self.S_LINE_TRACE_TO_ENTRY_CORNER
        self.get_logger().info(
            "앞으로 3m 이동 완료 → +Y 진입선 라인트레이싱 시작, L자 코너 탐색"
        )
